...
app.use('/api/v1', (() => {
  ...
  router.post('/analyze', async (req, res) => {
    ...
    try {
      ...
      const cyclomaticComplexity = Math.max(1, (code.match(/if|else|for|while|switch|case|catch/g) || []).length);
      const maintainabilityIndex = Math.max(0, Math.min(100, 171 - 5.2 * Math.log(linesOfCode) - 0.23 * cyclomaticComplexity - 16.2 * Math.log(linesOfCode)));
      ...
      const analysisResult = {
        ...
        data: {
          ...
          documentation: {
            ...
            missingDocs: functions.map((f: any) => `Function: ${f.name}`),
          },
          complexity: {
            cyclomaticComplexity,
          ...
          maintainability: {
            ...
            grade: maintainabilityIndex >= 80 ? 'A' : 
  ...
  router.get('/analysis/:hash', async (req, res) => {
    try {
      ...
      // Check database
      if (pgPool) {
        const result = await pgPool.query('SELECT * FROM analysis_results WHERE request_hash = $1', [hash]);
