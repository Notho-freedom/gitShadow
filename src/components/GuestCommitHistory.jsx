...
export default function GuestCommitHistory({ commits, onCommitSelect, selectedCommit, onShowUpgradeModal }) {
  ...
  const [showLimit, setShowLimit] = useState(isAuthenticated ? 30 : 5); // Plus de commits si connecté
  ...
  const handleLoadMore = () => {
    if (!isAuthenticated && onShowUpgradeModal) {
      // Si pas connecté et qu'on a une fonction pour afficher la modale d'upgrade, l'utiliser
      onShowUpgradeModal();
    } else {
      // Sinon, charger plus de commits (comportement par défaut)
      setShowLimit(prev => Math.min(prev + (isAuthenticated ? 30 : 5), filteredCommits.length));
    }
  };
        ...
        <div className="text-center pt-3">
          ...
          >
            {onShowUpgradeModal ? (
              ...
              `Charger plus (${Math.min(isAuthenticated ? 30 : 5, filteredCommits.length - showLimit)} restants)`
            )}
          </button>
        </div>
      )}

      {/* Guest Notice - seulement si pas connecté */}
      {!isAuthenticated && (
        <div className="mt-3 p-2 bg-yellow-500/10 border border-yellow-500/20 rounded">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-yellow-400">
              Mode invité : Affichage limité aux 5 commits les plus récents. Connectez-vous pour l'historique complet.
            </span>
          </div>
        </div>
      )}

      {/* Notice utilisateur connecté */}
      {isAuthenticated && (
        <div className="mt-3 p-2 bg-green-500/10 border border-green-500/20 rounded">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-green-400">
              Connecté : Accès complet à l'historique des commits et toutes les fonctionnalités.
            </span>
          </div>
        </div>
      )}
